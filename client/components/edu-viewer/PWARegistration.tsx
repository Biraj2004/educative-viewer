...
export default function PWARegistration() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && "serviceWorker" in navigator) {
      ...
      const registerSW = async () => {
        try {
          ...
          if (!probe.ok) {
            console.warn(`[PWA] Skipping Service Worker registration. ${swUrl} returned ${probe.status}.`);
            return;
          }
    ...
    if (typeof window !== "undefined") {
      ...
      if (isIosDevice) {
        if (!isDismissed) {
          // Show iOS instruction prompt automatically after a short delay
  ...
  useEffect(() => {
    ...
    const handleTriggerInstall = () => {
      console.log("[PWA] trigger-pwa-install event received");
      localStorage.removeItem("ev_pwa_dismissed"); // Clear dismissal state

      const isIosDevice =
        /iPad|iPhone|iPod/.test(navigator.userAgent) ||
        (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1);

      console.log("[PWA] isIosDevice:", isIosDevice, "userAgent:", navigator.userAgent, "platform:", navigator.platform, "maxTouchPoints:", navigator?.maxTouchPoints);

      if (isIosDevice) {
        console.log("[PWA] Setting showIosPrompt to true");
        setShowIosPrompt(true);
      } else if (deferredPrompt) {
        console.log("[PWA] Invoking programmatic install prompt");
        handleInstallClick();
      } else {
        console.log("[PWA] Setting showInstallPrompt to true");
        setShowInstallPrompt(true);
      }
    };
  ...
  return (
    <div
      ...
      className={`fixed bottom-4 left-4 right-4 md:left-auto md:right-4 md:w-96 z-[9999] p-5 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-2xl backdrop-blur-xl flex flex-col gap-4 transition-all duration-300 transform ${
        visible
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <button
          ...
          className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors cursor-pointer"
          aria-label="Dismiss"
        >
      ...
      {showInstallPrompt && (
        <div className="flex flex-col gap-2">
          {deferredPrompt ? (
            ...
            <div className="bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-xl p-3.5 text-xs text-gray-600 dark:text-gray-300 leading-relaxed flex flex-col gap-2">
              <span className="font-semibold text-indigo-600 dark:text-indigo-400">
                To install on Android or Desktop:
              </span>
              <ol className="list-decimal pl-4 space-y-1.5 text-gray-500 dark:text-gray-400">
                <li>
                  Open your browser&apos;s menu (tap the <span className="font-semibold text-gray-800 dark:text-gray-200">three dots</span> icon in the corner).
                </li>
                <li>
                  Choose <span className="font-semibold text-gray-800 dark:text-gray-200">Install app</span> or <span className="font-semibold text-gray-800 dark:text-gray-200">Add to Home screen</span>.
                </li>
              </ol>
            </div>
          )}
