...
| `NEXT_PUBLIC_RSA_PUBLIC_KEY` | Backend logs show an RSA private/public key message, Copy the public key value here |
| `NEXT_PUBLIC_BACKEND_API_BASE` | Browser API base URL for backend calls (use the backend URL directly by default; not the Cloudflare static worker URL) |
| `NEXT_PUBLIC_STATIC_FILES_BASE` | Static files base URL (usually `https://your-domain.com/api`) |
| `NEXT_PUBLIC_STATIC_BASIC_AUTH` | Basic auth header value for protected static Worker URLs.(Optional) |
...
### Backend API Routing Note

By default, the frontend should call the backend directly. Backend API traffic is **not** routed through Cloudflare in this setup.

Routing backend API calls through Cloudflare is possible, but it requires worker logic changes so the worker can distinguish:
- static asset requests (that should go to `staticBase`), and
- backend API requests (that should go to the backend service).

Without that route-detection logic, using the same prefix can send API calls to the wrong upstream.
